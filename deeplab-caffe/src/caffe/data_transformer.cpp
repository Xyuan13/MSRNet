...
//------------------
template<typename Dtype>
void DataTransformer<Dtype>::TransformImgAndCls(
    const cv::Mat& cv_img,
    const int img_cls, 
    Blob<Dtype>* transformed_data_blob,
    Blob<Dtype>* transformed_label_blob,
  ...
  CHECK_EQ(label_channels, 1);
  CHECK_EQ(label_height, 1);
  CHECK_EQ(label_width, 1);
  ...
  // for label
  top_index = 0;
  Dtype cls = static_cast<Dtype>(img_cls);
  transformed_label[top_index] = cls;
