function [rec, prec, maxF] = DrawPRCurve(SMAP, smapSuffix, GT, gtSuffix, targetIsFg, targetIsHigh, color, linestyle)
...
%     if recall ~= 0
%         feature = CalFmeasure(precision, recall, beta);
%         if feature > maxF
%             maxF = feature;
%         end
%     end
...
% F-meature
beta = 0.3;
Fmeasure = CalFmeasure(prec, rec, beta);
maxF = max(Fmeasure);
%             
