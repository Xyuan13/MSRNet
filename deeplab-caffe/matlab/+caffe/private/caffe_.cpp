...
/*static void solver_update(MEX_ARGS) {
  ...
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
    if (solver->is_net()) {
      ...
      solver->iter_increase();
      //  LOG(INFO) << "Compute updated values";
      solver->net()->Update(); // LOG(INFO) << "Network updated";    
  }
}*/
//------------------------
...
//-------------------------------------------
// Usage: caffe_('net_', hNet)
static void net_update(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('net_update', hNet)");
  Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
  net->Update();
}
//-------------------------------------------
